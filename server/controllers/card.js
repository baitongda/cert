...
exports.get = function* (next) {
  ...
  try {
    ...
    // 卡券使用限制
    if ((card.certLimit.length !== 0 && card.certLimit.indexOf(req.certId) < 0)) {
...
exports.validate = function* (certId, price, cardId, encryptCode) {
  ...
  if (price < card.leastCost) {
    return { errcode: 1, errmsg: "未达到优惠券使用金额" }
  }

  // 卡券使用限制
  if ((card.certLimit.length !== 0 && card.certLimit.indexOf(certId) < 0)) {
    // 不在使用范围内
    return { errcode: 1, errmsg: "该优惠券不适用于该商品" }
  }

  // 减免后价格
  price -= card.reduceCost;
  ...
  let order = null;
  try {
    order = yield Order.findOne({state: 2, cardCode: code}).exec();
  } catch(e) {
    return { errcode: 1, errmsg: e }
  }
...
// 核销卡券
exports.consume = function* (code) {
  try {
    let wx = yield Wechat.findOne().exec();
    res = yield reqwest({
      url: WX_CARD_URL.consume + wx.TOKEN,
      method: "POST",
      data: JSON.stringify({
        code : code
      })
    ...
    // console.log(res);
    if (res.errcode !== 0) {
      console.log("[consume err] " + res.errmsg);
    }
  } catch(e) {
